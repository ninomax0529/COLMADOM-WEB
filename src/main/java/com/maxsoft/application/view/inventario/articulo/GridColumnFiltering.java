...
//         Button btnIrAVista2 = new Button("Ir a Vista 2", e -> 
//            UI.getCurrent().navigate(RegistroEntradaDeIventarioView.class, getArticulo())
//        );
...
//                        -> ui.navigate("inventario/registroEntrada"))
//        );
//         
        Grid<Articulo> grid = new Grid<>(Articulo.class, false);

        grid.addColumn(createPersonRenderer()).setHeader("Articulo").setFlexGrow(0)
                .setWidth("230px");

        grid.addColumn(Articulo::getCodigo).setHeader("Descripcion");
//        grid.addColumn(Articulo::getDescripcion).setHeader("Unidad");
//          Grid<Articulo> grid = new Grid<>(Articulo.class, false);

        List<Articulo> people = articuloService.getLista();
        GridListDataView<Articulo> dataView = grid.setItems(people);
